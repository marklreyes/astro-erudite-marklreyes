...
Dear WordPress, since '09 you've been there for me like no other. I'll always remember how this ecosystem got me into web development. Through MAMP/XAMPP (hi, XP), to mucking with `header.php` and `footer.php` to get styles or logic into a global context (probably wasn't necessary), fracking CSS through child themes with `!important` as unlimited ammo, to feasting on plugin after plugin like it's Thanksgiving, it's been pretty cool to see how this ecosystem has grown from being built on top of the [Personal Home Page](https://en.wikipedia.org/wiki/PHP) to what it is today.
...
This playbook is very specific. It works under the assumption that you've chosen your [theme](https://astro.build/themes/), stored your theme into version control such as GitHub (or GitLab, BitBucket, Azure DevOps) and opted in to [Netlify](https://www.netlify.com/) as your final destination.
...
* Otherwise, I went with [Cassidy's setup using Netlify Redirects with Astro](https://cassidoo.co/post/netlify-redirects-astro/) guided by [List all URLs](https://wordpress.com/plugins/list-all-urls).
...
I was able to retire Cloudflare, WP Engine (saved 💰) and WordPress safely upon change. That said, those aforementioned technologies are still **very good** but this domain no longer justified those specific tools.
...
- AI was used once. Specifically, in VS Code with my GitHub CoPilot extension generating suggestions when using the existing [Authors Card Component](https://github.com/jktrn/astro-erudite/blob/main/src/components/AuthorCard.astro) as my source of inspo to create my [Testimonials Page](/testimonials).
